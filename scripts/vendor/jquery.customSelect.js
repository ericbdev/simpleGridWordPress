(function( $ ) {
	...
	$.fn.customSelectMenu = function( options ) {
		...
		function updateMenu( selection ) {
			...
			hiddenInput.val( customOptionValue).attr('id', customMenuName+'_id').trigger('change');
		...
		return this.each(function() {
			...
			if( $this.find( ':selected' ) ) {
				...
				if( !selectedOptionValue ) {
					...
					newHiddenInput.val( selectedOptionValue ).attr('id', selectName+'_id').trigger('change');
				}
